...
class SafetyController(Node):
    ...
    def get_wall(self, msg):
        ...
        self.get_logger().info(f"MIN = {thetas[0] * 180 / 3.14}, MAX = {thetas[-1] * 180 / 3.14}")
        self.get_logger().info(f"SENSOR RANGE = [{msg.angle_min*180/3.14:.2f}, {msg.angle_max*180/3.14:.2f}]")
    ...
    def control(self, msg):
        ...
        if self.car_velocity:        
            ...
            self.get_logger().info(f"CLOSEST DISTANCE = {r:.2f} VELOCITY = {self.car_velocity:.2f}")

            # tune this
            closest_dist = self.car_velocity * 0.5+0.25
